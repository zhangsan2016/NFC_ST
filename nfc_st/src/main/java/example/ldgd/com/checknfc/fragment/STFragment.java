...
public class STFragment extends Fragment {
    ...
    protected void setTag() {
        if (myTag == null)
            myTag = ((STFragmentListener) getActivity()).getTag();
    }
    ...
    /**
     * The first paramater should be the ID of the string to show.
     * Some optional parameters can be passed to the string.
     * @param resource_id
     * @param formatArgs
     */
    protected void showToast(final int resource_id, Object... formatArgs) {

        // Show the Toast only if the Fragment is still attached to the Activity
        if (isAdded()) {
            Resources resources = getResources();
            if(resources != null) {
                final String message = resources.getString(resource_id, formatArgs);
