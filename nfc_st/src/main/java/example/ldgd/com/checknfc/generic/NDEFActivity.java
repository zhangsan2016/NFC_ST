...
import example.ldgd.com.checknfc.fragment.NDEFAarFragment;
import example.ldgd.com.checknfc.fragment.NDEFBtFragment;
import example.ldgd.com.checknfc.fragment.NDEFBtLeFragment;
import example.ldgd.com.checknfc.fragment.NDEFEditorFragment;
import example.ldgd.com.checknfc.fragment.NDEFEmailFragment;
import example.ldgd.com.checknfc.fragment.NDEFEmptyFragment;
import example.ldgd.com.checknfc.fragment.NDEFExternalFragment;
import example.ldgd.com.checknfc.fragment.NDEFMimeFragment;
import example.ldgd.com.checknfc.fragment.NDEFRecordFragment;
import example.ldgd.com.checknfc.fragment.NDEFSmsFragment;
import example.ldgd.com.checknfc.fragment.NDEFTextFragment;
import example.ldgd.com.checknfc.fragment.NDEFUnknownRecordFragment;
import example.ldgd.com.checknfc.fragment.NDEFUriFragment;
import example.ldgd.com.checknfc.fragment.NDEFVCardFragment;
import example.ldgd.com.checknfc.fragment.NDEFWifiFragment;
...
import static example.ldgd.com.checknfc.generic.NDEFActivity.ActionStatus.NDEF_MESSAGE_TOO_BIG;
import static example.ldgd.com.checknfc.generic.NDEFActivity.ActionStatus.TAG_NOT_IN_THE_FIELD;
...
public class NDEFActivity extends STFragmentActivity implements NavigationView.OnNavigationItemSelectedListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mNDEFRecord instanceof TextRecord) {
            ...
            mFragment = NDEFUnknownRecordFragment.newInstance(context);
        }
        ...
        if (drawer.isDrawerOpen(GravityCompat.START)) {
            ...
            switch(mAction) {
                case NDEFEditorFragment.EDIT_NDEF_RECORD:
                    ...
                    // set dialog message
                    alertDialogBuilder
                            .setMessage(getString(R.string.data_not_written_to_tag_leave_edition_mode))
                            .setCancelable(true)
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_save:
                if (mFragment != null) {
                    ...
                    showToast(R.string.internal_error_invalid_fragment);
                }
                return true;
            ...
            case R.id.action_delete:
                if(mAction == NDEFEditorFragment.ADD_NDEF_RECORD) {
                    ...
                    // set dialog message
                    alertDialogBuilder
                            .setMessage(getString(R.string.delete_record_question))
                            .setCancelable(true)
                            .setPositiveButton(getString(R.string.yes),new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int id) {
    ...
    private void showNdefTooBigAlert() {
        ...
        String message = getString(R.string.ndef_message_too_big);

        if(memoryAreaSizeInBytes != 0) {
            if (myTag instanceof STType4Tag) {
                message += "\n\n";
                message +=  getString(R.string.ndef_size_in_bytes,ndefSizeInBytes);
                message += "\n";
                message += getString(R.string.max_file_size_in_bytes,memoryAreaSizeInBytes);
                message += "\n";
                ...
                message += "\n\n";
                message +=  getString(R.string.ndef_cc_size_in_bytes,totalLength);
                message += "\n";
                message += getString(R.string.max_area_size_in_bytes,memoryAreaSizeInBytes);
                message += "\n";
            }
        }
    ...
    private class AsyncTaskWriteNdefMessage extends AsyncTask<Void, Void, ActionStatus> {
        STProgressDialog mUserWritingInformation;
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            mUserWritingInformation = showNdefWritingAlert();
        }

        @Override
        protected void onPostExecute(ActionStatus actionStatus) {
            if (mUserWritingInformation != null && mUserWritingInformation.isShowing()) {
                mUserWritingInformation.dismiss();
            }
    ...
    private class AsyncTaskDisplayPasswordDialogBoxForType5Tag extends AsyncTask<Void, Void, ActionStatus> {
        ...
        protected void onPostExecute(ActionStatus actionStatus) {
            ...
            switch(actionStatus) {
                case ACTION_SUCCESSFUL:
                    String dialogMsg = getResources().getString(R.string.enter_area_pwd, UIHelper.getAreaName(mArea));
